...
def require_dependency(name, import_error):
    if import_error is None:
        return
    raise RuntimeError(
        f"필수 Python 모듈 '{name}'을 import할 수 없습니다. "
        f"설치/환경 설정을 확인하세요. 원인: {import_error}"
    ) from import_error

def resolve_yolo_device(device="auto"):
...
def get_realsense_ids():
    ...
    """
    require_dependency("pyrealsense2", REALSENSE_IMPORT_ERROR)
    connected_devices = {}
...
def capture_realsense_data(serial_number, mode="mid_50", warmup_frames=10, visualize=False):
    ...
    try:
        ...
        raise RuntimeError(f"RealSense 프레임 캡처 실패: {e}") from e
...
def detect_objects_yolo(model, color_img_bgr, target_classes=None, visualize=False, device="auto"):
    """
    ...
    if target_classes is not None:
        results = model(color_img_bgr, classes=target_classes, device=yolo_device, verbose=False)
    else:
        results = model(color_img_bgr, device=yolo_device, verbose=False)
...
def get_target_grasp_pose(class_index, target_class_name, local_id=0):
    """
    ...
    poses = class_index[target_class_name]
    if local_id < 0 or local_id >= len(poses):
        print(
            f"🚨 [{target_class_name}] local_id={local_id}가 범위를 벗어났습니다. "
            f"가능 범위: 0~{len(poses) - 1}"
        )
        return None, None, None, None

    # class_index[target_class_name]가 이미 광학축 가까운 순서로 정렬되어 있어야 함
    target_brick = poses[local_id]
...
def search_bricks(
    ...
    require_dependency("open3d", OPEN3D_IMPORT_ERROR)
