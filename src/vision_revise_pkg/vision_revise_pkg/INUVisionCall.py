import os
from vision_revise_pkg import INUVisionLib as ivl
...
class VisionManager:
    ...
    def _load_model_once(self, kind):
        if kind == "brick":
            if self._brick_model is None:
                from ultralytics import YOLO
                print(f"[VISION_REVISE] 브릭 YOLO 모델 로드: {self.yolo_dir_brick}")
                self._brick_model = YOLO(self.yolo_dir_brick)
            return self._brick_model

        if kind == "component":
            if self._component_model is None:
                from ultralytics import YOLO
                print(f"[VISION_REVISE] 컴포넌트 YOLO 모델 로드: {self.yolo_dir_component}")
                self._component_model = YOLO(self.yolo_dir_component)
    ...
    def capture_camera(self, mode="mid_50", V_visualize=False):
        ...
        if self.color_rgb is None or self.depth is None or self.intrinsics is None or self.scale is None:
            raise RuntimeError(
                "RealSense 캡처 실패: color/depth/intrinsics/scale 중 None이 있습니다."
            )

        print(
            "[VISION_REVISE] 캡처 완료 | "
            f"color_shape={getattr(self.color_rgb, 'shape', None)}, "
            f"depth_shape={getattr(self.depth, 'shape', None)}, scale={self.scale}"
        )
    ...
    def run_search(self, mode, V_visualize=False):
        ...
        self.pose_table, self.class_index = ivl.search_bricks(
            mode,
            self.yolo_dir_brick,
            self.color_rgb,
            self.depth,
            ...
            yolo_model=self._load_model_once("brick"),
            yolo_device=self.device
        )

        print(
            "[VISION_REVISE] 브릭 탐색 완료 | "
            f"pose_count={len(self.pose_table or [])}, classes={list((self.class_index or {}).keys())}"
        )
    ...
    def run_search_assembly(self,V_visualize=False):
        ...
        self.pose_table, self.class_index = ivl.search_assembly(self.color_rgb,
                                                                ...
                                                                self.intrinsics,
                                                                self.scale,
                                                                yolo_model=self._load_model_once("component"),
