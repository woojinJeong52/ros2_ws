...
MODEL_PATH = r"C:\Users\HDL\OneDrive\문서\LAB\2026 로보컵\비전모델\Block_s_ver1.0\best.pt"
CAMERA_ID = 1
CAMERA_WIDTH = 1280
CAMERA_HEIGHT = 720
CAMERA_FPS = 30
...
SAVE_DIR = r"C:\Users\HDL\OneDrive\문서\LAB\2026 로보컵\비전모델\Block_s_ver1.0\RealTimeResults"
BLOCK_CLASS_NAMES = [
    "2x2_blue",
    "2x2_green",
    "2x2_red",
...
def get_class_count_text(result, model_names):
    if result.boxes is None:
        return []
    ...
    cls_values = result.boxes.cls.detach().cpu().numpy().astype(int).tolist()
...
def draw_info_panel(frame, fps_value, device, conf, model_path, detected_count, class_lines):
    text_lines = [
        f"FPS: {fps_value:.1f}",
        f"Device: {device}",
        f"Camera ID: {CAMERA_ID}",
        f"Conf Threshold: {conf:.2f}",
        f"Detected Blocks: {detected_count}",
        "Q or ESC: Quit",
        "S: Save Screenshot",
    ...
    if len(class_lines) > 0:
        text_lines.append("Detected Classes:")
        text_lines.extend(class_lines[:8])
...
def print_model_info(model, device):
    print("=" * 80)
    print("YOLOv8 Block Real-time Segmentation")
    print("=" * 80)
...
def main():
    ...
    window_name = "YOLOv8 Block Segmentation - Webcam"
    ...
    while True:
        ...
        class_lines = get_class_count_text(result, model.names)
        ...
        if key == ord("s") or key == ord("S"):
            ...
            save_path = save_dir / f"block_webcam_result_{timestamp}_{frame_idx:06d}.png"
