...
class MotorDriver(Node):
    def __init__(self):
        ...
        self.wheel_separation = 0.29  # Adjust as necessary
        ...
        self.timer = self.create_timer(0.1, self.recv_motor_state)
    ...
    def recv_motor_state(self):
        ...
        if len(data) == 0:
            self.get_logger().error("[recv_motor_state] Error: Receive data timeout")
            return
        exitflag = False
        ...
        if exitflag:
            self.get_logger().error("[recv_motor_state] Error: Invalid frame received")
            return
        else:
    ...
    def update_odometry(self):
        # RPM to m/s
        left_wheel_speed = (self.rpm1 / 60.0) * (np.pi * self.wheel_diameter)
        right_wheel_speed = (self.rpm2 / 60.0) * (np.pi * self.wheel_diameter)
