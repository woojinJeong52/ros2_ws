...
class Nodelet(Node):
    def __init__(self):
        ...
        self.gain_count = 2
        self.gain_list = [0.5, 0.7, 1.0, 1.3, 1.5, 1.7, 2.0, 2.3, 2.5, 3.3]
        ...
        self.linear_velocity_prev = 0.0
        self.angular_velocity_prev = 0.0
        ...
        self.vel_input4_prev = 0.0
        self.vel_input3_prev = 0.0
        self.vel_input4_filtered = 0.0
        self.vel_input3_filtered = 0.0
    ...
    def timer_callback(self):
        ...
        if self.JOY_CONTROL:
            ...
            # self.md.send_position_cmd(int(self.target_pos1), int(self.target_pos2), int(100000), int(100000))
            self.md.send_vel_cmd(self.vel_input3_filtered, self.vel_input4_filtered)
        ...
        scale_factor = 1  # mA로 표현
        self.msg_wheelmotor.current1 = int(self.md.current1 * scale_factor)
        self.msg_wheelmotor.current2 = int(self.md.current2 * scale_factor)

        # self.msg_wheelmotor.target1 = int(self.vel_input1)
        # self.msg_wheelmotor.target2 = int(self.vel_input2)


        v_x = (self.md.rpm1+self.md.rpm2)*np.pi*self.wheel_diameter/(60*2*4.33)
        w_z = (self.md.rpm2-self.md.rpm1)*np.pi*self.wheel_diameter/(60*self.wheel_separation*4.33)


        self.msg_wheelmotor.v_x = v_x * self.alpha_v + self.v_x_old*(1-self.alpha_v)
        self.msg_wheelmotor.w_z = w_z * self.alpha_v + self.w_z_old*(1-self.alpha_v)
        self.pub.publish(self.msg_wheelmotor)

        self.v_x_old = self.msg_wheelmotor.v_x
        self.w_z_old = self.msg_wheelmotor.w_z
        ...
        self.filtered_linear_velocity = (
            self.alpha_v * linear_velocity + (1 - self.alpha_v) * self.linear_velocity_prev
        )
        self.filtered_angular_velocity = (
            self.alpha_v * angular_velocity + (1 - self.alpha_v) * self.angular_velocity_prev
        )

        self.linear_velocity_prev = self.filtered_linear_velocity
        self.angular_velocity_prev = self.filtered_angular_velocity

        

        # Update pose
        self.pose_theta += self.msg_wheelmotor.w_z * dt
        if (self.pose_theta > 2*np.pi):
            self.pose_theta = self.pose_theta - (2 * np.pi)
        if (self.pose_theta < -2*np.pi):
            self.pose_theta = self.pose_theta + (2 * np.pi)


        self.pose_x += self.msg_wheelmotor.v_x * np.cos(self.pose_theta) *dt
        self.pose_y += self.msg_wheelmotor.v_x * np.sin(self.pose_theta) *dt
        ...
        odom_msg.pose.pose.position.z = self.pose_theta
        ...
        odom_msg.twist.twist.linear.x = self.msg_wheelmotor.v_x      #기존코드
        odom_msg.twist.twist.angular.z = self.msg_wheelmotor.w_z

        odom_msg.twist.twist.linear.y = float(self.md.current1)
        odom_msg.twist.twist.linear.z = float(self.md.current2)
    ...
    def cmd_vel_callback(self, msg):
        if not self.JOY_CONTROL:
            ...
            velocity_right = (linear_velocity + (self.wheel_separation / 2.0) * angular_velocity)*4.33
            velocity_left = (linear_velocity - (self.wheel_separation / 2.0) * angular_velocity)*4.33
            ...
            self.vel_input4 = (velocity_right / (self.wheel_diameter * np.pi )) * 60
            self.vel_input3 = (velocity_left / (self.wheel_diameter * np.pi )) * 60

            self.vel_input4_filtered = self.vel_input4 * self.alpha_velocity + self.vel_input4_prev * (1 - self.alpha_velocity)
            self.vel_input3_filtered = self.vel_input3 * self.alpha_velocity + self.vel_input3_prev * (1 - self.alpha_velocity)

            self.vel_input4_prev = self.vel_input4_filtered
            self.vel_input3_prev = self.vel_input3_filtered
