...
class Draw_MPC_tracking(object):
    def __init__(self, robot_states: list, init_state: np.array, robot_predict: list ,rob_diam=0.3, export_fig=False):
        self.init_state = init_state
        self.robot_states = robot_states
        self.robot_predict = robot_predict
        self.rob_radius = rob_diam
        self.fig = plt.figure()
        self.ax = plt.axes(xlim=(-1.0, 6), ylim=(-0.5, 1.5))
        # self.fig.set_size_inches(7, 6.5)
        # init for plot
        self.animation_init()
    ...
    def animation_init(self, ):
        # draw target line
        self.target_line = plt.plot([0, 6], [1, 1], '-r')
        ...
        # 예측 경로 그리기
        self.predict_lines = []
        for prediction in self.robot_predict:
            line, = self.ax.plot([], [], 'b--')  # Empty plot which will be updated
            self.predict_lines.append(line)
        return self.target_line, self.robot_body, self.robot_arr, *self.predict_lines
    ...
    def animation_loop(self, indx):
        ...
        self.robot_arr = mpatches.Arrow(position[0], position[1], self.rob_radius * np.cos(orientation),
                                        self.rob_radius * np.sin(orientation), width=0.2, color='r')
        self.ax.add_patch(self.robot_arr)
        # for line, predict in zip(self.predict_lines, self.robot_predict[indx]):
        #     line.set_data(predict[0], predict[1])
        # return self.robot_arr, self.robot_body, *self.predict_lines
        return self.robot_arr, self.robot_body
